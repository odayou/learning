...
[assembly: InternalsVisibleTo("Unity.Cloud.Foundation.Tests")]
[assembly: InternalsVisibleTo("Unity.Cloud.Foundation.EditorTests")]
namespace UnityEngine.Cloud.Foundation
{
    /// <summary>
    /// Class to handle active loader and subsystem management for Cloud. This class is to be added as a
    /// ScriptableObject asset in your project and should only be referenced by the <see cref="CloudGeneralSettings"/>
    ...
    ///
    /// Depending on configuration the <see cref="CloudManagerSettings"/> instance will automatically manage the active loader
    ...
    /// of the Cloud session normally handled by the <see cref="CloudManagerSettings"/> instance. Toggling this to false also toggles
    ...
    /// </summary>
    public sealed class CloudManagerSettings : ScriptableObject
    {
        [HideInInspector]
...
#pragma warning restore 414
        ...
        [SerializeField]
        [Tooltip("Determines if the Cloud Manager instance is responsible for creating and destroying the appropriate loader instance.")]
        ...
        [SerializeField]
        [Tooltip("Determines if the Cloud Manager instance is responsible for starting and stopping subsystems for the active loader instance.")]
        ...
        [SerializeField]
        [Tooltip("List of Cloud Loader instances arranged in desired load order.")]
        [FormerlySerializedAs("Loaders")]
        List<CloudLoader> m_Loaders = new List<CloudLoader>();

        // Maintains a list of registered loaders that is immutable at runtime.
        [SerializeField]
        [HideInInspector]
        HashSet<CloudLoader> m_RegisteredLoaders = new HashSet<CloudLoader>();

        /// <summary>
        /// List of loaders currently managed by this Cloud Manager instance.
        /// </summary>
        /// <remarks>
        ...
        /// </remarks>
        [Obsolete("'CloudManagerSettings.loaders' property is obsolete. Use 'CloudManagerSettings.activeLoaders' instead to get a list of the current loaders.")]
        public List<CloudLoader> loaders
        {
        ...
        /// <summary>
        /// A shallow copy of the list of loaders currently managed by this Cloud Manager instance.
        /// </summary>
        /// <remarks>
        /// This property returns a read only list. Any changes made to the list itself will not affect the list
        /// used by this Cloud Manager instance. To mutate the list of loaders currently managed by this instance,
        /// use <see cref="TryAddLoader"/>, <see cref="TryRemoveLoader"/>, and/or <see cref="TrySetLoaders"/>.
        /// </remarks>
        public IReadOnlyList<CloudLoader> activeLoaders => m_Loaders;

        /// <summary>
        /// Read only boolean letting us know if initialization is completed. Because initialization is
        /// handled as a Coroutine, people taking advantage of the auto-lifecycle management of CloudManager
        ...
        public void InitializeLoaderSync()
        {
            ...
            {
                Debug.LogWarning(
                    "Cloud Management has already initialized an active loader in this scene." +
        ...
        /// </summary>
        /// <param name="loader">
        /// The <see cref="CloudLoader"/> to be added to this manager's instance of loaders.
        /// </param>
        /// <param name="index">
        /// The index at which the given <see cref="CloudLoader"/> should be added. If you set a negative or otherwise
        ...
        /// Play mode, attempting to add an <see cref="CloudLoader"/> will always succeed and register that loader's type
        ...
        /// returns <c>false</c>.
        /// </remarks>
        public bool TryAddLoader(CloudLoader loader, int index = -1, bool initialize = false)
        {
        ...
        /// </summary>
        /// <param name="loader">
        /// The <see cref="CloudLoader"/> to be removed from this manager's instance of loaders.
        /// </param>
        /// <returns>
        ...
        /// </remarks>
        public bool TryRemoveLoader(CloudLoader loader)
        {
        ...
        /// The list of <see cref="CloudLoader"/>s to be managed by this manager instance.
        /// </param>
        /// <returns>
        ...
        /// runtime/Play mode, the new loader list will be validated against the registered <see cref="CloudLoader"/> types.
        /// If any loaders exist in the list that were not registered at startup, the attempt will fail.
        /// </remarks>
        public bool TrySetLoaders(List<CloudLoader> reorderedLoaders)
        {
            var originalLoaders = new List<CloudLoader>(activeLoaders);
#if UNITY_EDITOR
            if (!EditorApplication.isPlaying)
            {
                registeredLoaders.Clear();
                currentLoaders.Clear();
                for (var index = 0; index < reorderedLoaders.Count; index++)
                {
                    var loader = reorderedLoaders[index];
                    if (!TryAddLoader(loader, index, index == 0 && !EditorApplication.isPlayingOrWillChangePlaymode))
                    {
        ...
        private bool CheckGraphicsAPICompatibility(CloudLoader loader)
        {
            ...
            {
                Debug.LogWarning(String.Format("The {0} does not support the initialized graphics device, {1}. Please change the preffered Graphics API in PlayerSettings. Attempting to start the next Cloud loader.", loader.name, deviceType.ToString()));
                return false;
            }
        ...
        internal List<CloudLoader> currentLoaders
        {
            get { return m_Loaders; }
            set { m_Loaders = value; }
        }
        ...
        // hashset of loaders.
        internal HashSet<CloudLoader> registeredLoaders
        {
