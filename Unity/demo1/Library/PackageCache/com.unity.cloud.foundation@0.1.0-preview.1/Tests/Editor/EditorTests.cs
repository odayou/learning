...
using Unity.Cloud.TestTooling;

namespace UnityEditor.Cloud.Foundation.Tests
{

    class CloudGeneralSettingsTests : ManagementTestSetup
    {
        ...
        {
            bool success = CloudGeneralSettingsUpgrade.UpgradeSettingsToPerBuildTarget(testPathToSettings);
            Assert.IsTrue(success);

            CloudGeneralSettingsPerBuildTarget pbtgs = null;

            pbtgs = AssetDatabase.LoadAssetAtPath(testPathToSettings, typeof(CloudGeneralSettingsPerBuildTarget)) as CloudGeneralSettingsPerBuildTarget;
    ...
    [TestFixture(3)]
    [TestFixture(4)]
    class EditorActiveLoadersManipulationTests : CloudSettingsManagerTestBase
    {
        ...
        public void CheckIfIllegalSetLoaderListFails()
        {
            ...
            var invalidList = new List<CloudLoader>(manager.activeLoaders) { dl, dl };
