...
{
    internal class CloudLoaderInfoManager : ICloudLoaderOrderManager
    {
        ...
        List<CloudLoaderInfo> m_AllLoaderInfos = new List<CloudLoaderInfo>();
        List<CloudLoaderInfo> m_AllLoaderInfosForBuildTarget = new List<CloudLoaderInfo>();
        List<CloudLoaderInfo> m_AssignedLoaderInfos = new List<CloudLoaderInfo>();
        List<CloudLoaderInfo> m_UnassignedLoaderInfos = new List<CloudLoaderInfo>();
        ...
        void PopulateAllLoaderInfos()
        {
            ...
            CloudLoaderInfo.GetAllKnownLoaderInfos(m_AllLoaderInfos);
        }
        ...
        void PopulateAssignedLoaderInfos()
        {
            ...
            for (int i = 0; i < m_LoaderList.arraySize; i++)
            {
                ...
                info.instance = prop.objectReferenceValue as CloudLoader;
        ...
        private List<CloudLoaderInfo> FilteredLoaderInfos(List<CloudLoaderInfo> loaderInfos)
        {
            List<CloudLoaderInfo> ret = new List<CloudLoaderInfo>();
            ...
            foreach (var info in loaderInfos)
            {
                ...
                try
                {
                    attrs = info.loaderType.GetCustomAttributes(typeof(CloudSupportedBuildTargetAttribute), true);
                }
                catch (Exception)
                {
                    attrs = default;
                ...
                {
                    foreach (CloudSupportedBuildTargetAttribute attr in attrs)
                    {
        ...
        void UpdateSerializedProperty()
        {
            if (m_LoaderList != null && m_LoaderList.isArray)
            {
                ...
                int index = 0;
                foreach (CloudLoaderInfo info in m_AssignedLoaderInfos)
                {
        ...
        #region ICloudLoaderOrderManager
        List<CloudLoaderInfo> ICloudLoaderOrderManager.AssignedLoaders { get { return m_AssignedLoaderInfos; } }
        List<CloudLoaderInfo> ICloudLoaderOrderManager.UnassignedLoaders { get { return m_UnassignedLoaderInfos; } }

        void ICloudLoaderOrderManager.AssignLoader(CloudLoaderInfo assignedInfo)
        {
        ...
        void ICloudLoaderOrderManager.UnassignLoader(CloudLoaderInfo unassignedInfo)
        {
        ...
        void ICloudLoaderOrderManager.Update()
        {
            UpdateSerializedProperty();
            ShouldReload = true;
        }
