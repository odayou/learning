...
namespace UnityEditor.Cloud.Foundation
{
    ...
    {
        static readonly string kCloudBootSettingsKey = "Cloud-boot-settings";
        ...
        internal void ReadBootConfig()
        {
            ...
            string CloudBootSettings = UnityEditor.EditorUserBuildSettings.GetPlatformSettings(buildTargetName, kCloudBootSettingsKey);
            if (!String.IsNullOrEmpty(CloudBootSettings))
            {
                // boot settings string format
                // <boot setting>:<value>[;<boot setting>:<value>]*
                var bootSettings = CloudBootSettings.Split(';');
        ...
        internal void WriteBootConfig()
        {
            ...
            string buildTargetName = BuildPipeline.GetBuildTargetName(buildReport.summary.platform);
            EditorUserBuildSettings.SetPlatformSettings(buildTargetName, kCloudBootSettingsKey, sb.ToString());
        }
    }

    class CloudGeneralBuildProcessor : IPreprocessBuildWithReport, IPostprocessBuildWithReport
    {
        static readonly string kPreInitLibraryKey = "Cloudsdk-pre-init-library";

        class PreInitInfo
        {
            public PreInitInfo(ICloudLoaderPreInit loader, BuildTarget buildTarget, BuildTargetGroup buildTargetGroup)
            {
                this.loader = loader;
            ...
            public ICloudLoaderPreInit loader;
        ...
        {
            BuildHelpers.CleanOldSettings<CloudGeneralSettings>();
        }
        ...
        public void OnPreprocessBuild(BuildReport report)
        {
            ...
            CloudGeneralSettings settings = buildTargetSettings.SettingsForBuildTarget(report.summary.platformGroup);
            if (settings == null)
                return;

            CloudManagerSettings loaderManager = settings.AssignedSettings;
            ...
            if (loaderManager != null)
            {
              ...
              //  var summary = report.summary;
               ...
               // CloudManagementAnalytics.SendBuildEvent(summary.guid, summary.platform, summary.platformGroup, loaders);
                ...
                if (loaders.Count >= 1)
                {
                    preInitInfo = new PreInitInfo(loaders[0] as ICloudLoaderPreInit, report.summary.platform, report.summary.platformGroup);
                }
        ...
        public static void VerifyGraphicsAPICompatibility(CloudManagerSettings loaderManager, GraphicsDeviceType selectedDeviceType)
        {
