...
namespace UnityEditor.Cloud.Foundation
{
    ...
    internal interface ICloudLoaderOrderManager
    {
        List<CloudLoaderInfo> AssignedLoaders { get; }
        List<CloudLoaderInfo> UnassignedLoaders { get; }

        void AssignLoader(CloudLoaderInfo assignedInfo);
        void UnassignLoader(CloudLoaderInfo unassignedInfo);
        void Update();
    }

    internal class CloudLoaderOrderUI
    {
        struct LoaderInformation
        {
            ...
            public ICloudCustomLoaderUI customLoaderUI;
        }

        const string k_AtNoLoaderInstance = "There are no Cloud plugins applicable to this platform.";
        ...
        struct Content
        {
            public static readonly string k_HelpUri = "https://docs.unity3d.com/Packages/com.unity.Cloud.management@4.0/manual/EndUser.html";
            ...
            public static readonly GUIContent k_HelpContent = new GUIContent("",
                ...
                "Selecting an Cloud Plug-in Provider installs and loads the corresponding package in your project. You can view and manage these packages in the Package Manager.");
        }
        ...
        static void PopulateDeprecationInfo()
        {
            ...
            s_DeprecationInfo[k_DeprecatedWMRLoaderName] =  new DeprecationInfo{
                ...
                renderContent = new GUIContent("",
                    ...
                    @"Microsoft has transitioned support of Windows MR devices to OpenCloud in Unity 2021, and recommends using Unity's OpenCloud plugin. As such, this Windows Cloud plugin is marked as deprecated and will be removed in the 2021.2 release. It will continue to be supported in the 2020 LTS.")
            };
#endif //UNITY_2021_1_OR_NEWER
        }

        internal CloudLoaderOrderUI()
        {
        }
        ...
        void DrawElementCallback(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            li.toggled = CloudPackageMetadataStore.IsLoaderAssigned(li.loaderType, CurrentBuildTargetGroup);
        ...
        internal bool OnGUI(BuildTargetGroup buildTargetGroup)
        {
            ...
            if (buildTargetGroup != CurrentBuildTargetGroup || m_LoaderMetadata == null)
            {
                ...
                foreach (var pmd in CloudPackageMetadataStore.GetLoadersForBuildTarget(buildTargetGroup))
                {
                    ...
                    var newLi = new LoaderInformation() {
                        ...
                        toggled = CloudPackageMetadataStore.IsLoaderAssigned(pmd.loaderType, buildTargetGroup),
                        disabled = false,
                        customLoaderUI = CloudCustomLoaderUIManager.GetCustomLoaderUI(pmd.loaderType, buildTargetGroup)
                        };
                ...
                if (settings != null)
                {
                    ...
                    for (int i = 0; i < m_LoaderMetadata.Count; i++)
                    {
                        ...
                        if (CloudPackageMetadataStore.IsLoaderAssigned(settings.AssignedSettings, li.loaderType))
                        {
                            li.toggled = true;
                ...
                {
                    var status = CloudPackageMetadataStore.GetCurrentStatusDisplayText();
                ...
                if (settings != null)
                {
                    ...
                    for (int i = 0; i < m_LoaderMetadata.Count; i++)
                    {
                        ...
                        if (li.stateChanged)
                        {
                            if (li.toggled)
                            {
                                CloudPackageMetadataStore.InstallPackageAndAssignLoaderForBuildTarget(li.packageId, li.loaderType, buildTargetGroup);
                            }
                            else
                            {
                                CloudPackageMetadataStore.RemoveLoader(settings.AssignedSettings, li.loaderType, buildTargetGroup);
                            }
