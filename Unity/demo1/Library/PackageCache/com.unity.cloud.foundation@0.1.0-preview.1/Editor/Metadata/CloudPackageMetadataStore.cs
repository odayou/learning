...
namespace UnityEditor.Cloud.Foundation.Metadata
{
    ...
    /// to/from an <see cref="CloudManagerSettings"/> instance.
    /// </summary>
    [InitializeOnLoad]
    public class CloudPackageMetadataStore
    {
        const string k_WaitingPackmanQuery = "CloudMGT Waiting Packman Query.";
        const string k_RebuildCache = "CloudMGT Rebuilding Cache.";
        const string k_InstallingPackage = "CloudMGT Installing Cloud Package.";
        const string k_AssigningPackage = "CloudMGT Assigning Cloud Package.";
        const string k_UninstallingPackage = "CloudMGT Uninstalling Cloud Package.";
        const string k_CachedMDStoreKey = "Cloud Metadata Store";
...
#pragma warning disable CS0649
        ...
        static Dictionary<string, ICloudPackage> s_Packages = new Dictionary<string, ICloudPackage>();
        ...
        {
            return SessionState.GetString(queueName, k_DefaultSessionStateString) != CloudPackageMetadataStore.k_DefaultSessionStateString;
        }

        internal static bool isCheckingInstallationRequirements => CloudPackageMetadataStore.SessionStateHasStoredData(k_WaitingPackmanQuery);
        internal static bool isRebuildingCache => CloudPackageMetadataStore.SessionStateHasStoredData(k_RebuildCache);
        internal static bool isInstallingPackages => CloudPackageMetadataStore.SessionStateHasStoredData(k_InstallingPackage);
        internal static bool isUninstallingPackages => CloudPackageMetadataStore.SessionStateHasStoredData(k_UninstallingPackage);
        internal static bool isAssigningLoaders => CloudPackageMetadataStore.SessionStateHasStoredData(k_AssigningPackage);
        ...
        internal static ICloudPackageMetadata GetMetadataForPackage(string packageId)
        {
        ...
        internal static bool IsLoaderAssigned(string loaderTypeName, BuildTargetGroup buildTargetGroup)
        {
            ...
            var settings = CloudGeneralSettingsPerBuildTarget.CloudGeneralSettingsForBuildTarget(buildTargetGroup);
            if (settings == null)
                return false;
        ...
        internal static bool IsLoaderAssigned(CloudManagerSettings settings, string loaderTypeName)
        {
            if (settings == null)
                return false;
        ...
        /// </summary>
        /// <param name="settings">An instance of <see cref="CloudManagerSettings"/> to add the loader to.</param>
        ...
        public static bool AssignLoader(CloudManagerSettings settings, string loaderTypeName, BuildTargetGroup buildTargetGroup)
        {
            if (EditorApplication.isPlaying || EditorApplication.isPaused)
            {
                Debug.LogError($"Attempt to add {loaderTypeName} for {buildTargetGroup} while in Play mode. Cloud Foundation can not make changes to the loader list when running.");
                return false;
            }

            var instance = EditorUtilities.GetInstanceOfTypeWithNameFromAssetDatabase(loaderTypeName);
            if (instance == null || !(instance is CloudLoader))
            {
        ...
        public static bool RemoveLoader(CloudManagerSettings settings, string loaderTypeName, BuildTargetGroup buildTargetGroup)
        {
            if (EditorApplication.isPlaying || EditorApplication.isPaused)
            {
                Debug.LogError($"Attempt to remove {loaderTypeName} for {buildTargetGroup} while in Play mode. Cloud Foundation can not make changes to the loader list when running.");
                return false;
            }
            ...
            CloudLoader loader = instance as CloudLoader;
            ...
            if (settings.TryRemoveLoader(loader))
            {
                ...
                AssetDatabase.SaveAssets();
#if UNITY_EDITOR
                var loaderHelper = loader as CloudLoaderHelper;
        ...
        internal static ICloudPackage GetPackageForSettingsTypeNamed(string settingsTypeName)
        {
        ...
        {
            if (CloudPackageMetadataStore.isCheckingInstallationRequirements)
            {
                return "Checking installation requirements for packages...";
            }
            else if (CloudPackageMetadataStore.isRebuildingCache)
            {
                return "Querying Package Manager for currently installed packages...";
            }
            else if (CloudPackageMetadataStore.isInstallingPackages)
            {
                return "Installing packages...";
            }
            else if (CloudPackageMetadataStore.isUninstallingPackages)
            {
                return "Uninstalling packages...";
            }
            else if (CloudPackageMetadataStore.isAssigningLoaders)
            {
        ...
        internal static void AddPluginPackage(ICloudPackage package)
        {
        ...
        static void InternalAddPluginPackage(ICloudPackage package)
        {
        ...
        static CloudPackageMetadataStore()
        {
        ...
        static void AddRequestToQueue(LoaderAssignmentRequest request, string queueName)
        {
            ...
            if (CloudPackageMetadataStore.SessionStateHasStoredData(queueName))
            {
        ...
        static void AssignAnyRequestedLoadersUpdate()
        {
            ...
            while (reqs.activeRequests.Count > 0)
            {
                ...
                var settings = CloudGeneralSettingsPerBuildTarget.CloudGeneralSettingsForBuildTarget(req.buildTargetGroup);
                ...
                {
                    var assignedSettings = ScriptableObject.CreateInstance<CloudManagerSettings>() as CloudManagerSettings;
                    settings.AssignedSettings = assignedSettings;
                    EditorUtility.SetDirty(settings);
                }

                if (!CloudPackageMetadataStore.AssignLoader(settings.AssignedSettings, req.loaderType, req.buildTargetGroup))
                {
                    req.installationState = InstallationState.Log;
        ...
        static void QueueLoaderRequest(LoaderAssignmentRequest req)
        {
            switch (req.installationState)
            {
                ...
                case InstallationState.Log:
                    const string header = "Cloud Foundation";
                    switch(req.logLevel)
                    {
                        case LogLevel.Info:
